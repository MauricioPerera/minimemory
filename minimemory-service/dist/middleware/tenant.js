import { hasAccessToTenant, getUserRoleInTenant } from './jwt';
/**
 * Tenant extraction middleware
 * Extracts tenant ID from X-Tenant-Id header or API key binding
 */
export async function extractTenant(c, next) {
 ...
 */
export function requireTenantRole(minRoles) {
    return async (c, next) => {
        const tenantId = c.get('tenantId');
        const authMethod = c.get('authMethod');
        if (!tenantId) {
 ...
 */
export async function validateNamespaceAccess(c, namespace) {
    const tenantId = c.get('tenantId');
    if (!tenantId) {
 ...
 */
export async function checkTenantLimits(c, type) {
    const tenantId = c.get('tenantId');
    if (!tenantId) {
