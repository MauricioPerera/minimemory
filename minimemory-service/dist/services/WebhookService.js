...
export class WebhookService {
    ...
    async create(options) {
        ...
        await this.db
            ...
            .bind(webhook.id, webhook.namespace, webhook.tenantId || null, webhook.url, webhook.secret, JSON.stringify(webhook.events), webhook.isActive ? 1 : 0, webhook.description || null, webhook.maxRetries, webhook.retryBackoffMs, webhook.successCount, webhook.failureCount, webhook.createdAt, webhook.updatedAt)
            .run();
        return webhook;
    }
    /**
    ...
    async list(namespace, options) {
        ...
        const results = await this.db
            .prepare(`SELECT * FROM webhooks WHERE ${whereClause}
    ...
    async trigger(namespace, eventType, data, tenantId) {
        ...
        await this.db
            .prepare(`UPDATE webhooks SET last_triggered_at = ? WHERE id IN (${webhookIds.map(() => '?').join(', ')})`)
    ...
    async getDeliveries(options = {}) {
        ...
        // Get deliveries
        const results = await this.db
            .prepare(`SELECT * FROM webhook_deliveries ${whereClause}
				ORDER BY created_at DESC LIMIT ? OFFSET ?`)
            .bind(...params, limit, offset)
            .all();
 ...
 */
export function createWebhookTrigger(db, ctx) {
