...
import { KnowledgeService, type TextChunk } from '../services/KnowledgeService.js';
			...
			for (let i = 0; i < chunks.length; i++) {
				...
				await db
					...
					.bind(
						...
						0.5, // Default importance for knowledge
						JSON.stringify({
							sourceId: source.id,
							sourceName: name,
							sourceType: type,
...
knowledgeRoutes.get('/sources', async (c) => {
	const namespace = getNamespace(c);
	const type = c.req.query('type') as KnowledgeSourceType | undefined;
...
knowledgeRoutes.post('/chunk-preview', async (c) => {
	try {
		const body = await c.req.json();
		const { content, chunking = {} } = body as {
			content: string;
			chunking?: ChunkingOptions;
		};
