...
import { AuditService, type AuditQueryOptions } from '../services/index.js';
 ...
 */
function getAuditService(c: { env?: Bindings }): AuditService | null {
...
auditRoutes.get('/resource/:type/:id', async (c) => {
	...
	try {
		const resourceType = c.req.param('type') as 'memory' | 'namespace' | 'user' | 'tenant' | 'session' | 'api_key';
