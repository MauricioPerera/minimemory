 ...
 */
function getNamespace(c: { req: { header: (name: string) => string | undefined } }): string {
 ...
 */
function getTenantId(c: { req: { header: (name: string) => string | undefined } }): string | undefined {
 ...
 */
function validateEvents(events: unknown): WebhookEventType[] | null {
	if (!Array.isArray(events)) return null;
	if (events.length === 0) return null;

	for (const event of events) {
		if (typeof event !== 'string') return null;
		if (!WEBHOOK_EVENTS.includes(event as WebhookEventType)) return null;
	}
...
webhookRoutes.put('/:id', async (c) => {
	...
	try {
		const body = await c.req.json();
		const updates: UpdateWebhookOptions = {};
