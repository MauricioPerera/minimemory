...
auth.post('/login', async (c) => {
  try {
    ...
    const user = await db
      ...
      .bind(email.toLowerCase())
      .first<{ id: string; email: string; password_hash: string; name: string | null; is_active: number }>();
    ...
    const tenantsResult = await db
      ...
      `)
      .bind(user.id)
      .all<{ id: string; name: string; role: 'owner' | 'admin' | 'member' | 'viewer' }>();

    const tenants: TenantInfo[] = tenantsResult.results.map(t => ({
      id: t.id,
      name: t.name,
      role: t.role,
    }));
    ...
    const accessToken = await createAccessToken(
      { sub: user.id, email: user.email, name: user.name || '', tenants },
      c.env.JWT_SECRET
    );
...
auth.post('/refresh', async (c) => {
  try {
    ...
    const newAccessToken = await createAccessToken(
      { sub: session.user_id, email: session.email, name: session.name || '', tenants },
      c.env.JWT_SECRET
    );
...
auth.get('/me', requireJwtAuth, async (c) => {
  try {
    ...
    const user = await db
      ...
      .bind(userId)
      .first<{ id: string; email: string; name: string | null; created_at: number; last_login: number | null }>();
...
/**
 * POST /auth/validate-agent
 * Validate API key + agent token combination for MCP authentication
 * This is a public endpoint used by MCP servers to validate connections
 */
auth.post('/validate-agent', async (c) => {
  try {
    const body = await c.req.json();
    const { apiKey, agentToken } = body as { apiKey?: string; agentToken?: string };
