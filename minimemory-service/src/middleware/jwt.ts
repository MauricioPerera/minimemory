...
import { verifyAccessToken, AccessTokenPayload, TenantInfo } from '../utils/tokens';
 ...
 */
export async function requireJwtAuth(c: Context<{ Bindings: Env }>, next: Next) {
 ...
 */
export function hasAccessToTenant(c: Context, tenantId: string): boolean {
 ...
 */
export function getUserRoleInTenant(
  c: Context,
  tenantId: string
